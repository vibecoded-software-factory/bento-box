...
class I3Workspace: public QObject {
	Q_OBJECT;
	QML_ELEMENT;
	QML_UNCREATABLE("i3 is not the compositor on macOS");

public:
	explicit I3Workspace(QObject* p = nullptr): QObject(p) {}
};
...
class I3IpcListener: public QObject {
	Q_OBJECT;
	Q_PROPERTY(
	    QStringList subscriptions READ subscriptions WRITE setSubscriptions NOTIFY
